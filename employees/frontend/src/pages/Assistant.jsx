...
export default function Assistant() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: "Hi! I can list pending approvals, summarize a client's activity, give you a bank-wide summary, or propose an approval decision. What do you need?" },
  ])
  ...
  async function sendMessage(e) {
    ...
    try {
      const res = await api.post('/agent/employee/chat', { message: input })
  ...
  async function handleConfirm(index, actionId) {
    try {
      await api.post(`/agent/employee/actions/${actionId}/confirm`)
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Decision applied.' }])
  ...
  async function handleReject(index, actionId) {
    try {
      await api.post(`/agent/employee/actions/${actionId}/reject`)
  ...
  return (
    <Layout>
      <h1 className="font-display text-2xl font-semibold text-steel-900 mb-6">Assistant</h1>

      <div className="bg-white rounded-2xl border border-slate-300/40 flex flex-col h-[calc(100vh-180px)] max-w-2xl">
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                msg.role === 'user' ? 'bg-steel-900 text-white' : 'bg-paper-50 text-steel-900'
              }`}>
                ...
                {pendingActions[i] && (
                  <div className="flex gap-2 mt-3">
                    ...
                    <button onClick={() => handleReject(i, pendingActions[i])} className="px-3 py-1.5 border border-slate-300 text-steel-900 rounded-lg text-xs font-medium hover:bg-white transition">Cancel</button>
                  </div>
                )}
              </div>
            </div>
          ))}
          {loading && <div className="flex justify-start"><div className="bg-paper-50 rounded-2xl px-4 py-2.5 text-sm text-slate-500">Thinking...</div></div>}
          <div ref={bottomRef} />
        </div>

        <form onSubmit={sendMessage} className="border-t border-slate-300/40 p-3 flex gap-2">
          <input
            type="text" value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="Ask about approvals, clients, or bank stats..."
