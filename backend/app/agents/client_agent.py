...
client_agent = Agent(
    ...
    system_prompt=(
        ...
        "For the client's OWN account (the sender side of a transfer), prefer "
        "referring to it by type - 'checking' or 'savings'. If the client has "
        "multiple accounts of the same type, disambiguate using nicknames. "
        "For the RECIPIENT of a transfer, use the recipient's email address "
        "to look up their account automatically. If they have multiple "
        "accounts, disambiguate using nicknames, never account numbers. "
